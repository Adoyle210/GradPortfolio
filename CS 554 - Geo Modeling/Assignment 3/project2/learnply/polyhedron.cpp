...
Polyhedron::Polyhedron(FILE *file)
{
	...
	{
		printf("Error: Could not read first line of PLY file\n");
		return;
	}
	if (strncmp(buf, "ply", 3) != 0)
	{
		printf("Error: File does not start with 'ply'\n");
		return;
	}
	rewind(file);
	...
	if (!in_ply)
	{
		printf("Error: Failed to read PLY file - invalid PLY format\n");
		return;
	}

	// Print PLY file information
	printf("PLY file info:\n");
	printf("  Number of element types: %d\n", in_ply->num_elem_types);
	printf("  File type: %d\n", in_ply->file_type);
	...
	if (!vlist || !tlist)
	{
		printf("Error: Failed to allocate memory for vertices or triangles\n");
		return;
	}
	...
	for (i = 0; i < in_ply->num_elem_types; i++)
	{
		...
		if (!elem_name)
		{
			printf("Error: Failed to setup element read for type %d\n", i);
			continue;
		}
		printf("Reading element type: %s (count: %d)\n", elem_name, elem_count);
		...
		if (equal_strings("vertex", elem_name))
		{
			...
			if (!vlist)
			{
				printf("Error: Failed to allocate vertex list\n");
				return;
			}
			printf("Allocated space for %d vertices\n", nverts);

			// Set up for getting vertex elements
			printf("Setting up vertex properties:\n");
			...
			for (int k = 0; k < 4; k++)
			{
				...
				{
					printf("Error: Failed to setup vertex property %s\n", vert_props[k].name);
					return;
				}
			}
			...
			for (j = 0; j < nverts; j++)
			{
				...
				if (!vlist[j])
				{
					printf("Error: Failed to create vertex %d\n", j);
					continue;
				}
				vlist[j]->valence_deficit = vert.angle_deficit;
			...
			printf("Successfully read %d vertices\n", nverts);
		}
		else if (equal_strings("face", elem_name))
		{
			...
			if (!tlist)
			{
				printf("Error: Failed to allocate triangle list\n");
				return;
			}
			printf("Allocated space for %d triangles\n", ntris);

			// Set up for getting face elements
			printf("Setting up face properties:\n");
			...
			for (int k = 0; k < 2; k++)
			{
				...
				try
				{
					setup_property_ply(in_ply, &face_props[k]);
				}
				catch (...)
				{
					printf("Error: Failed to setup face property %s\n", face_props[k].name);
					return;
				}
			}
			...
			for (j = 0; j < ntris; j++)
			{
				...
				if (face.nverts != 3)
				{
					printf("Error: Face %d has %d vertices (should be three)\n", j, face.nverts);
					if (face.verts)
						free(face.verts);
					continue;
				}
				...
				if (!tlist[j])
				{
					printf("Error: Failed to create triangle %d\n", j);
					if (face.verts)
						free(face.verts);
					continue;
				}
		...
		{
			printf("Skipping unknown element type: %s\n", elem_name);
	...
	printf("Fixing up vertex pointers in triangles...\n");
	...
	for (i = 0; i < ntris; i++)
	{
		...
		for (j = 0; j < 3; j++)
		{
			int idx = (intptr_t)tlist[i]->verts[j];
			if (idx < 0 || idx >= nverts)
			{
				printf("Error: Invalid vertex index %d in triangle %d\n", idx, i);
				continue;
			}
			tlist[i]->verts[j] = vlist[idx];
		}
	}

	printf("Removing invalid triangles...\n");
	int valid_tris = 0;
	for (i = 0; i < ntris; i++)
	{
		if (!tlist[i])
		...
		else
		{
			if (valid_tris != i)
			{
				tlist[valid_tris] = tlist[i];
			}
			valid_tris++;
		}
	}
	ntris = valid_tris;
	printf("Final vertex count: %d, triangle count: %d\n", nverts, ntris);
}
...
void Polyhedron::compute_silhouette_edges(const icMatrix3x3 &view, const icVector3 &translate)
{
	...
	//find edges where faces switch directions 
	Edge* edge; 
...
void Polyhedron::compute_silhouette_faces(const icMatrix3x3 &view, const icVector3 &translate)
{
	...
	//tag all the verticies as facing forward or backward 
	Vertex* vert;
	icVector3 eye, norm;
	double dprod;
	for (int i = 0; i < nverts; i++)
	...
	for (int i = 0; i < ntris; i++)
	{
		...
		for (int j = 0; j < 3; j ++)
		{
			...
			{
				//find where the silhoutte crosses the edge using linear interopation 
