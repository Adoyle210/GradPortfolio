...
//	This is a twist OpenGL / GLUT program
//
...
bool SlowTwist = false;  // Toggle for twist speed
...
Display( )
{
	...
	Pattern.Use();
	Pattern.SetUniformVariable((char*)"uKa", 0.3f);
	Pattern.SetUniformVariable((char*)"uKd", 0.7f);
	...
	// Add this line to create the twisting animation
	float twistSpeed = SlowTwist ? 0.5f : 2.0f;
	float twistAmount = SlowTwist ? 1.0f : 2.0f;
	float twist = twistAmount * F_PI * sin(twistSpeed * F_PI * Time);
	Pattern.SetUniformVariable((char*)"uTwist", twist);

	glCallList(DL);  // Draw the twist object
...
InitGraphics( )
{
	...
	Pattern.Init( );
	bool valid = Pattern.Create( (char *)"twist.vert", (char *)"twist.frag" );
	if( !valid )
	...
	Pattern.SetUniformVariable( (char *)"uKs", 0.4f );
	Pattern.SetUniformVariable( (char *)"uColor", 0.7f, 0.9f, 0.3f, 1.0f );
	Pattern.SetUniformVariable( (char *)"uSpecularColor", 1.0f, 1.0f, 1.0f, 1.0f );
	Pattern.SetUniformVariable( (char *)"uShininess", 16.0f );
	Pattern.UnUse( );
	...
	fprintf(stderr, "Display List created with ID: %d\n", DL);
}
...
Keyboard( unsigned char c, int x, int y )
{
	...
	case 'F':
		...
		case 't':
		case 'T':
			SlowTwist = !SlowTwist;  // Toggle between fast and slow
			break;

		case 'c':
		case 'C':
			UseChromaDepth = !UseChromaDepth;
			break;
