    ...
    bool read_from_data_file(istream& in) {
        ...
        if (bytes_read == 4096) {
            ...
            for (int i = 0; i < num; i++) {
                ...
                cursor += sizeof(id);
 // int - manager id
                memcpy(reinterpret_cast<char*>(&manager_id), page_data + cursor, sizeof(manager_id));
                cursor += sizeof(manager_id);
                ...
                memcpy(name, page_data + cursor, name_size);
                ...
                // cursor += bio_size;
                // str - bio
                char* bio = new char[bio_size + 1];
                memcpy(bio, page_data + cursor, bio_size);
                bio[bio_size] = '\0'; // Null-terminate the string
                cursor += bio_size;
