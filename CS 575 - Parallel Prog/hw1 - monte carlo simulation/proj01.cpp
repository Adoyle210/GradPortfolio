...
main( int argc, char *argv[ ] )
{
	...
	//fprintf( stderr, "OpenMP is supported -- version = %d\n", _OPENMP );
		...
		// note note: if you are windows, take the ", stderr" out of this list!
			...
			float da = Length( (PinAx - holeaxs[n]),  (PinAy - holeays[n]) );
			if( (da + PinAr) <= holears[n] )
			{
				float db = Length( (PinBx - holebxs[n]),  (PinBy - holebys[n]) );
				if( (db + PinBr) <= holebrs[n] )
				{
					float dc = Length( (PinCx - holecxs[n]),  (PinCy - holecys[n]) );
					if( (dc + PinCr) <= holecrs[n])
                                		numSuccesses++;
				}
			}
